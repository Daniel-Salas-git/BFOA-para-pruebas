...
def run_experiment():
    ...
    for _ in range(iteraciones):  # número de iteraciones  
        for bacteria_instance in poblacion:
            bacteria_instance.tumboNado(tumbo)
            bacteria_instance.mejorarFitness()
        chemio.doChemioTaxis(poblacion, dAttr, wAttr, hRep, wRep)
        globalNFE += chemio.parcialNFE 
        best = max(poblacion, key=lambda x: x.fitness)
        if (bestBacteria is None) or (best.fitness > bestBacteria.fitness):
            bestBacteria = Bacteria(path)
            clonaBest(bestBacteria, best)
        chemio.eliminarClonar(path, poblacion)
...
for ronda in range(num_rondas):
    fitness, nfe, bestBacteria = run_experiment()
    resultados.append((fitness, nfe, bestBacteria))
    print(f"Ronda {ronda+1}: Fitness = {fitness:.2f}, NFE = {nfe}")

# Ordenar y mostrar los mejores resultados
resultados.sort(reverse=True, key=lambda x: x[0])  # Ordena por fitness descendente

print("\n--- Mejores resultados de cada ronda ---")
for idx, (fitness, nfe, _) in enumerate(resultados):
    print(f"Ronda {idx+1}: Fitness = {fitness:.2f}, NFE = {nfe}")
